...
<div class="display categories category">
	<div class="floaticon">
		{if $print_page ne 'y'}
			{* =-=- CUSTOM BEGIN: icons -=-= *}
			...
			{if $gContent->hasUpdatePermission()}
				<a title="{tr}Edit this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.CATEGORIES_PKG_URL}edit_category.php?category_id={$gContent->mInfo.category_id}">{biticon ipackage="icons" iname="accessories-text-editor" iexplain="Edit `$gContent->mType.content_name`"}</a>
			{/if}
			{if $gContent->hasExpungePermission()}
				<a title="{tr}Remove this {$gContent->getContentTypeName()|strtolower}{/tr}" href="{$smarty.const.CATEGORIES_PKG_URL}remove_category.php?category_id={$gContent->mInfo.category_id}">{biticon ipackage="icons" iname="edit-delete" iexplain="Remove Category"}</a>
			{/if}
		{/if}<!-- end print_page -->
	</div><!-- end .floaticon -->
